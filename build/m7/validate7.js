 ...
 * estrutura · engine ≡ UI (tcd, emaxModel, tcdLayout) · curva FENa×dose pintada ≡
 * tcdLayout() · caso ≥8 · trilha ≥13 · dois bancos · figura viva ≥8 · cromo.
 * Guarda farmacológica INVERTIDA (§8): EXIGE tiazídicos com dose+unidade (mg) ancorados
 * ao NCC, dose-resposta computada e nota de honestidade do modelo.
 */
var fs = require('fs');
...
var ids = [
  ...
  'tcd-canvas', 'in-gfr', 'in-gitelman', 'in-droga', 'in-dose',
  'out-ncc', 'out-fena', 'out-ca', 'out-k', 'out-na', 'out-pca', 'out-classe',
  'veredito', 'instr-pearl', 'lab-pearl', 'fig-caso', 'dose-box',
  'tutor-q', 'tutor-opts', 'tutor-fb', 'tutor-score', 'tutor-total', 'tutor-fig',
  'draw-curva', 'draw-ca', 'draw-dose'
];
...
ok(doc.getElementById('tcd-canvas').tagName.toLowerCase() === 'canvas', 'instrumento: canvas tcd-canvas');
...
ok(/c[áa]lcio/i.test(conc), 'Conceito: cálcio (paradoxo)');
ok(/tiaz/i.test(conc), 'Conceito: tiazídico');
...
var amostras = [
  {}, { droga: 'hidroclorotiazida', dose: 50 }, { droga: 'clortalidona', dose: 25 }, { droga: 'indapamida', dose: 2.5 },
  { droga: 'hidroclorotiazida', dose: 50, gfr: 20 }, { gitelman: true }, { droga: 'hidroclorotiazida', dose: 25, gitelman: true },
  { droga: 'xyz', dose: -5, gfr: NaN }
];
var divT = 0;
amostras.forEach(function (a) { if (JSON.stringify(win.tcd(a)) !== JSON.stringify(ref.tcd(a))) divT++; });
ok(divT === 0, 'engine ≡ UI: tcd inline idêntico ao model7.js (' + divT + ' divergências)');
var divE = 0;
[[0, 12.5, 0.9], [12.5, 12.5, 0.9], [25, 10, 0.9], [2.5, 1.25, 0.9]].forEach(function (t) {
...
var canvasEl = doc.getElementById('tcd-canvas');
var rec = canvasEl.getContext('2d');
ok(rec && Array.isArray(rec.__paths), 'canvas: contexto exercido');
var initState = { gfr: 120, gitelman: false, droga: 'hidroclorotiazida', dose: 25 };
...
ok(/\b\d+[\s–-]+\d+\s?mg/.test(body) || /\b\d+[.,]?\d*\s?mg\b/.test(body), 'farmacologia (EXIGIDO): doses com unidade mg presentes');
ok(/NCC/.test(body), 'farmacologia: mecanismo (NCC) ancorado');
ok(typeof win.emaxModel === 'function' && /nccBlock|Emax/i.test(html), 'farmacologia: dose-resposta computada (emaxModel/nccBlock)');
ok(ref.tcd({ droga: 'hidroclorotiazida', dose: 50 }).caUrinaria < ref.tcd({}).caUrinaria, 'farmacologia: paradoxo do Ca (retém Ca) computado');
